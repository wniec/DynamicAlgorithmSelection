...
class LMCMAES(Optimizer):
    ...
    def initialize(self, is_restart=False, x=None, y=None, mean=None):
        mean = mean if mean is not None else self._initialize_mean(is_restart)  # mean of Gaussian search distribution
        x = x if x is not None else np.empty((self.n_individuals, self.ndim_problem))  # offspring population
        ...
        b = np.empty((self.m,))
        d = np.empty((self.m,))
        y = y if y is not None else np.empty((self.n_individuals,))  # fitness (no evaluation)
